...
try:
    ...
    # argument "readings" is not required default value is 1
    data = hx.get_raw_data_mean(readings=1)
    ...
    # do some kind of loop and do not pass any argument. Default 'readings' is 1
    ...
    input(
        'That is why I recommend always passing argument times=10 or higher value'
    )
    for i in range(40):
    ...
    print('Current offset for channel A and the current gain (64): ' +
          str(offset))
    offset = hx.get_current_offset(
        channel='B')  # for channel B no argument gain_A
    ...
    for i in range(2):
        ...
        if result:
            # without argument default is 1
            print('-> Weight channel A gain 128: ' +
                  str(hx.get_weight_mean(10)) + ' g')
            print('-> Raw data channel A gain 128: ' +
                  str(hx.get_raw_data_mean(10)))
            print('--------------------------------------------')
        else:
        ...
        if result:
            ...
            # without argument default is 1
            print('-> Weight channel B gain 32: ' +
                  str(hx.get_weight_mean(10)) + ' g\n')
            print('-> Raw data channel B gain 32: ' +
                  str(hx.get_raw_data_mean()))
        else:
        ...
        print('I remember last raw data for channel A gain 64: ' +
              str(last_value))
        last_value = hx.get_last_raw_data(channel='A', gain_A=128)
        print('I remember last raw data for channel A gain 128: ' +
              str(last_value) + '\n')
